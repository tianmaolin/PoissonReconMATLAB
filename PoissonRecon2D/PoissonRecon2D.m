function [C, implic] = poissonRecon2D(ptCloud2d, minDepth, maxDepth, ...
  verbose, normalRatio)
%PoissonRecon2D Perform the Poisson Surface Reconstruction algorithm on 2-D
% point cloud. And support quadtree adaptive to the normalfield.
%
...
% normalRatio: normalfield adaptive ratio to accelarating original Poisson recon
...
if verbose
  ...
  figure
  grad = resize(b, [sqrt(size(b, 1)), sqrt(size(b, 1))]);
  mesh(grad)
  title('b')
...
K = 0.045 * ones(5);
Z = conv2(Z, K, 'same'); % TODO: isoValue(Z)
iso_value = isoValueGrid(Z, samples, maxDepth);
implic = Z;
implic(Z > iso_value) = 1;
implic(Z <= iso_value) = 0;
figure
